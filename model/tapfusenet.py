...
from .block import CLIB, DetailEnhancement


class TAPFuseNet(nn.Module):
    def __init__(self, img_size = 512, norm = nn.BatchNorm2d, act = nn.ReLU):
        ...
        self.fusion_block = CLIB(norm = norm, act = act)
    ...
    def forward(self, img):
        ...
        img_feature, side_logits = self.fusion_block(features_list)   # <- CHANGED
        # upsample side logits to GT size (img_size × img_size)
        if isinstance(side_logits, (list, tuple)):
            side_logits = [
                F.interpolate(s, size=(self.img_size, self.img_size),
        ...
        return mask, coarse_mask, side_logits      # <- CHANGED
